...
TOP::  build_hashes $(ALLOBJS)

CORE::  build_hashes $(ALLOBJS)
	cd closure
	nmake /nologo /f NMakefile.mak PROLOG="$(PROLOG)" PROLOGDIR="$(PROLOGDIR)"
	cd ..\includes
	nmake /nologo /f NMakefile.mak PROLOG="$(PROLOG)" PROLOGDIR="$(PROLOGDIR)"
...
build_hashes :
	-@if exist cc\ergoact\ergoact_check.P (  \
	  cd cc\ergoact  &&  \
	  "$(PROLOG)" -e "[ergo_hashes],dump_hashes,halt."   &&  \
	  cd ..\..   \
	)
...
CLEAN : CLEANTEMP
	...
	-@if exist .flora_paths.bat  del /AH .flora_paths.bat
	-@if exist .#* erase .#*
