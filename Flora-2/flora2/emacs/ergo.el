;;; flora.el --- a major mode for editing and running Flora-2 and Ergo programs
...
;; This package provides a major mode for editing Flora-2 and Ergo programs.
;; It knows about Flora-2 and Ergo syntax and comments, and can send
...
;;   (autoload 'flora-mode "flora" "Major mode for editing and running Flora-2 programs." t)
;;   (setq flora-program-path "path-to-your-runflora")
;;
;;  ** For Ergo:
;;   (setq auto-mode-alist (cons '("\\(\\.flr$\\|\\.ergo$\\|\\.ergotxt$\\)" . ergo-mode) auto-mode-alist))
;;   (autoload 'ergo-mode "flora" "Major mode for editing and running Ergo programs." t)
...
(defconst flora-program-path "~/FLORA/flora2/runflora"
  "*Program name for invoking an inferior Flora-2 with `run-flora'.")
(defconst ergo-program-path "~/FLORA/flora2/runergo"
  "*Program name for invoking an inferior Ergo with `run-ergo'.")
(defvar flora-program-name nil
  "*Program name for invoking an inferior process with `run-ergo' or
`rin-flora'. Internal.")

(defvar flora-command-line nil
  "*Flora-2 or Ergo command to execute at startup.")
...
(defconst flora-process-buffer-const "*flora2*"
  "Name of the Flora-2 buffer.")
(defconst flora-process-name-const "flora2"
...
(defconst flora-directives-regexp
  ...
  "Flora-2/Ergo compiler directives without the \\( and \\).")
...
(defconst flora-font-lock-keywords
   (list
    ...
    '("\\b\\(\\\+\\|true\\|false\\|undefined\\|avg\\|max\\|min\\|sum\\|count\\|avgdistinct\\|sumdistinct\\|countdistinct\\|setof\\|bagof\\|load\||add\||insert\\|delete\\|b?t_?insert\\|b?t_?delete\\|insertall\\|b?t_?insertall\\|deleteall\\|b?t_?deleteall\\|erase\\|eraseall\\|b?t_?erase\\|b?t_?eraseall\\|insertrule_?[az]?\\|deleterule\\|\\[a-z]+\\|p2h\\|semantics\||setsemantics\\|caller\\|newoid\\|test\\|catch\\|clause\\|must\\|wish\\|exists?\\|forall\\|newmodule\\|erasemodule\\|t?enable\\|t?disable\\|is[a-z]+\\)\\b[ \t]*{"
      1 'font-lock-keyword-face)
    '("\\b\\(must\\|wish\\|exists?\\|forall\\)\\b[ \t]*("
      1 'font-lock-keyword-face)
    '("\\(\\\\[A-Za-z#]+\\)"
      1 'font-lock-keyword-face)
    '("\\(@!\\|@\\|@@\\)"
  ...
  "Additional expressions to highlight in Flora-2/Ergo mode.")
...
(defconst flora-infix-connective
  "\\(;\\|\\\\or\\|\\\\and\\|,\\|<==>?\\|==>\\|<~~>?\\|~~>\\)"
  "Regexp matching a binary Ergo/Flora-2 connective.")

(defvar flora-mode-menu
  '(:visible (eq major-mode 'flora-mode)
    ["Load Flora-2 file"    flora-load-file-to-module   t]
    ["Load Flora-2 buffer"  flora-load-buffer-to-module t]
    ["Load Flora-2 region"  flora-load-region-to-module t]
    "---"
    ["Add Flora-2 file"    flora-add-file-to-module   t]
    ["Add Flora-2 buffer"  flora-add-buffer-to-module t]
    ["Add Flora-2 region"  flora-add-region-to-module t]
    "---"
    ["Execute region as Flora-2 query"  flora-send-region-as-query t]
    "---"
...
(defvar ergo-mode-menu
  '(:visible (eq major-mode 'ergo-mode)
    ...
    ["Restart Ergo process"   flora-restart	    t]
    "---"
    ["Interrupt Ergo process" flora-interrupt	    t]
    ["Quit Ergo process"      flora-quit    	    t]
    ))
...
if that value is non-nil."
  ...
  (setq flora-process-buffer flora-process-buffer-const
        flora-process-name   flora-process-name-const
        flora-program-name   flora-program-path
...
if that value is non-nil."
  ...
  (setq flora-process-buffer ergo-process-buffer-const
        ...
        flora-program-name   ergo-program-path
  ;; Set up Ergo menus
  (if window-system
...
(defun flora-indent-line (&optional whole-exp)
  "Indent current line as Flora-2/Ergo code.
...
(defun flora-indent-level ()
  "Compute Flora-2/Ergo indentation level."
  (save-excursion
    (beginning-of-line)
    (skip-chars-forward " \t")
    (cond
...
(defun flora-comment-indent ()
  "Compute Flora-2/Ergo style comment indentation."
  (cond ((looking-at "///") 0)
...
;;;
;;; Inferior Flora-2/Ergo mode
;;;
(defvar inferior-flora-mode-map nil)

(defun inferior-flora-mode ()
  "Major mode for interacting with an inferior Flora-2/Ergo process.
...
\\[comint-stop-subjob] stops. \\[comint-quit-subjob] sends quit signal."
  ...
  (comint-mode)
  (flora-mode-variables)
  (if (string= flora-process-name "ergo")
    ...
    (setq major-mode 'comint-mode
          mode-name "Flora-2 process"
          comint-prompt-regexp "^flora2 \\?- *"
          comint-input-ring-file-name (expand-file-name "~/.flora2-history")))
...
;;;###autoload
(defun run-flora ()
  "Run a Flora-2 process. Input and output via buffer *flora2*, and
...
switch to the buffer."
  (interactive)
  ;; for now same as run-flora
  (run-flora-background)
  (show-flora-buffer 'switch))


(defun flora-load-region (&optional beg end module)
  "Send the region to the Flora-2 or Ergo process.
...
(defun flora-send-region-as-query (&optional beg end)
  "Send the region to the Flora-2 or Ergo process as a query.
...
(defun flora-load-buffer (&optional module)
  "Send the current buffer to the Flora-2 or Ergo process.
...
(defun flora-load-file (&optional file module add)
  "Prompt for a file, offer to save all buffers, then run Flora-2 or Ergo
on the file."
  (interactive "P")
...
;; SWITCH means switch to the inferior Flora-2/Ergo buffer
